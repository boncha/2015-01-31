## First we load the data and we give the names of the columns
 features<-read.table("./features.txt",header=F,stringsAsFactors = F) ## this df contains the names of the columns for x_train and x_test dfs.

 subject_train<-read.table("./train/subject_train.txt",header=F,stringsAsFactors = F,sep="") ## this df contains the IDs of the subjects mathcing the x_train measurement
 y_train<-read.table("./train/y_train.txt",header=F,stringsAsFactors = F,sep="") ## this df contains the activities for the subjects mathcing the x_train measurement
 x_train<-read.table("./train/X_train.txt",header=F,stringsAsFactors = F,sep="") ## this df cotains the measurements for the train subjects
 names(x_train)<-features$V2 ## asignes the cloumn names to the x_train data
 names(y_train)<-"Activity" ## asignes the cloumn name to the activity_train df
 names(subject_train)<-"SubjectID"  ## asignes the cloumn name to the subject_train df

 subject_test<-read.table("./test/subject_test.txt",header=F,stringsAsFactors = F,sep="") ## this df contains the IDs of the subjects mathcing the x_test measurement
 y_test<-read.table("./test/y_test.txt",header=F,stringsAsFactors = F,sep="") ## this df contains the activities for the subjects mathcing the x_test measurement
 x_test<-read.table("./test/X_test.txt",header=F,stringsAsFactors = F,sep="") ## this df cotains the measurements for the test subjects
  names(x_test)<-features$V2 ## asignes the cloumn names to the x_test data
  names(y_test)<-"Activity" ## asignes the cloumn name to the activity_test df
  names(subject_test)<-"SubjectID"  ## asignes the cloumn name to the subject_test df

## Then we combine the subject ID, Activity, and measurements for the train data, into a new df: train_data
 train_data<-cbind(subject_train,y_train,x_train)
## Then we combine the subject ID, Activity, and measurements for the test data, into a new df: test_data
...
## And we rename the columns in the Activity_labels df:
